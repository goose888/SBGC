...
readin_model <- TRUE
classification <- ''  # SoilOrder, Ecoregions, Landcover, SoilEcoregions, SoilLandcover


if(readin_model) {
  model_fname <- 'socprofeq.dat'
#  model_fnamefor <- 'socprofeq_force1.dat'
  model_fnamefree <- 'socprofeq_free10.dat'
  model_fnamefreet <- 'socprofeq_freet.dat'
  model_fnamefreew <- 'socprofeq_freew.dat'
...
if(readin_model) {
  ...
  soc_free <- t(read.table(model_fnamefree, sep="", na.strings='-9999.', fill=TRUE))
  soc_freet <- t(read.table(model_fnamefreet, sep="", na.strings='-9999.', fill=TRUE))
  soc_freew <- t(read.table(model_fnamefreew, sep="", na.strings='-9999.', fill=TRUE))
  ...
  soc_den_free <- soc_free / model_ldepth
  soc_den_freet <- soc_freet / model_ldepth
  soc_den_freew <- soc_freew / model_ldepth
  ...
  if(group_model){
     if(group_veg){
        ...
        model_sd <- model_mean
        model_ci <- model_mean
        model_free_mean <- model_mean
        model_free_sd <- model_mean
        model_free_ci <- model_mean
        model_freet_mean <- model_mean
        model_freet_sd <- model_mean
        model_freet_ci <- model_mean
        model_freew_mean <- model_mean
        model_freew_sd <- model_mean
        model_freew_ci <- model_mean
        cnt <- 1
        ...
        for (i in as.numeric(veglevel)) {
           model_subset <- soc_den_free[,veg_case == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_free_mean[,cnt] <- model_subset
           } else {
              model_free_mean[,cnt] <- rowMeans(model_subset, na.rm=TRUE)
              model_free_sd[,cnt] <- apply(model_subset, 1, sd, na.rm=TRUE)
              model_free_ci[,cnt] <- 1.96 * model_free_sd[,cnt] / sqrt(rowSums(!is.na(model_subset)))
           }
           rm(model_subset)
           cnt <- cnt + 1
        }
        for (i in as.numeric(veglevel)) {
           model_subset <- soc_den_freet[,veg_case == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_freet_mean[,cnt] <- model_subset
           } else {
              model_freet_mean[,cnt] <- rowMeans(model_subset, na.rm=TRUE)
              model_freet_sd[,cnt] <- apply(model_subset, 1, sd, na.rm=TRUE)
              model_freet_ci[,cnt] <- 1.96 * model_freet_sd[,cnt] / sqrt(rowSums(!is.na(model_subset)))
           }
           rm(model_subset)
           cnt <- cnt + 1
        }
        for (i in as.numeric(veglevel)) {
           model_subset <- soc_den_freew[,veg_case == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_freew_mean[,cnt] <- model_subset
           } else {
              model_freew_mean[,cnt] <- rowMeans(model_subset, na.rm=TRUE)
              model_freew_sd[,cnt] <- apply(model_subset, 1, sd, na.rm=TRUE)
              model_freew_ci[,cnt] <- 1.96 * model_freew_sd[,cnt] / sqrt(rowSums(!is.na(model_subset)))
           }
           rm(model_subset)
           cnt <- cnt + 1
        }
        ...
        cnt <- 1
        for (i in as.numeric(ecolevel)) {
           model_subset <- soc_den_free[,eid_modeled == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_free_mean[,cnt] <- model_subset
           } else {
        ...
        cnt <- 1
        for (i in as.numeric(ecolevel)) {
           model_subset <- soc_den_freet[,eid_modeled == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_freet_mean[,cnt] <- model_subset
           } else {
        ...
        cnt <- 1
        for (i in as.numeric(ecolevel)) {
           model_subset <- soc_den_freew[,eid_modeled == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_freew_mean[,cnt] <- model_subset
           } else {
...
if(group_data & group_model & group_figure){
   ...
   if(group_veg) {
    ...
    #  legend(108, 135, legend = c("   Soil Samples"), cex=3, pch = 17, bty = "n")
     ...
     # pdf(file="Grouped_ecoregion.pdf", width=17, height=10)
      pdf(file="Free_sep.pdf", width=17, height=10)
      ### Failed to capture SOC density
      # Set margin
      par(omi=c(0,0.3,0,0), mar=c(2,8,8,2)) # all plots on one page
      # 
      plot(gelisol_mean[,2], depth_obs, xlab="", ylab="", #xlab = "Cumulative root fraction", ylab = "Soil depth (cm)", 
                  xlim=c(0,140), ylim=rev(c(0,160)), pch=17, cex= 0, xaxt='n', yaxt='n')
 ...
 #     lines(gelisol_mean[,3]-gelisol_sd[,3], depth_obs, lwd = 4, xaxt='n', yaxt='n', col="deepskyblue")
      ...
      lines(model_mean[,2], depth_model, lty = 3, lwd = 8, pch=17, cex=1, col="red", xaxt='n', yaxt='n')    # Ecoregion 5= Arctic Tundra
      lines(model_mean[,3], depth_model, lty = 3, lwd = 8, pch=17, cex=1, col="green", xaxt='n', yaxt='n')    # Ecoregion 8 =Intermontane Boreal
      #lines(c_1_mean+c_1_se, depth, lwd = 4, xaxt='n', yaxt='n', col="gray50")
      #lines(c_1_mean-c_1_se, depth, lwd = 4, xaxt='n', yaxt='n', col="gray50")
    #  lines(model_mean[,4], depth_model, lty = 6, lwd = 4, col="blue", xaxt='n', yaxt='n')    # Ecoregion 6 = Bering Tundra
   
      lines(model_free_mean[,2], depth_model, lty = 1, lwd = 4, pch=11, cex=1, col="red", xaxt='n', yaxt='n')    # Ecoregion 5= Arctic Tundra
      lines(model_free_mean[,3], depth_model, lty = 1, lwd = 4, pch=11, cex=1, col="green", xaxt='n', yaxt='n')    # Ecoregion 8 =Intermontane Boreal
      lines(model_freet_mean[,2], depth_model, lty = 2, lwd = 4, pch=14, cex=1, col="red", xaxt='n', yaxt='n')    # Ecoregion 5= Arctic Tundra
      lines(model_freet_mean[,3], depth_model, lty = 2, lwd = 4, pch=14, cex=1, col="green", xaxt='n', yaxt='n')    # Ecoregion 8 =Intermontane Boreal
      lines(model_freew_mean[,2], depth_model, lty = 6, lwd = 4, pch=14, cex=1, col="red", xaxt='n', yaxt='n')    # Ecoregion 5= Arctic Tundra
      lines(model_freew_mean[,3], depth_model, lty = 6, lwd = 4, pch=14, cex=1, col="green", xaxt='n', yaxt='n')    # Ecoregion 8 =Intermontane Boreal
   ...
   #   legend(60, 160, legend = c("Model - Arctic Tundra", "Model - Bering Tundra", "Model - Intermontane Boreal"), cex=3, lty=6, col=c("red","blue","green"), lwd=3, bty = "n")#, lty = 1:7,
      legend(90, 50, legend = c("Historical, Tundra", "Historical, Boreal"), cex=3, lty=3, col=c("red","green"), lwd=3, bty = "n")#, lty = 1:7,
      legend(90, 75, legend = c("1D coupled, Tundra", "1D coupled, Boreal"), cex=3, lty=1, col=c("red","green"), lwd=3, bty = "n")#, lty = 1:7,
      legend(90, 100, legend = c("1D T coupled, Tundra", "1D T coupled, Boreal"), cex=3, lty=2, col=c("red","green"), lwd=3, bty = "n")#, lty = 1:7,
      legend(90, 125, legend = c("1D W coupled, Tundra", "1D coupled, Boreal"), cex=3, lty=6, col=c("red","green"), lwd=3, bty = "n")#, lty = 1:7,
           #  pch = "*", ncol = 4, cex = 0.8)
      # Put Label for X and Y axis
      mtext("Soil Depth (cm)", side=2, outer=TRUE, line=-2, cex=3)
      mtext("Organic Carbon Density (kgC/m3)", side=3, outer=TRUE,line=-3.7,cex=3)
