...
if(readin_model) {
  ...
  model_fnamefor <- 'socprofeq_force1.dat'   # socprofeq_force10.dat
  model_fnamefree <- 'socprofeq_free1.dat'   # socprofeq_free10.dat
...
if(readin_model) {
  ...
  soc_for <- t(read.table(model_fnamefor, sep="", na.strings='-9999.', fill=TRUE))
  ...
  soc_den_for <- soc_for / model_ldepth
  ...
  if(group_model){
     if(group_veg){
        ...
        model_sd <- model_mean
        model_ci <- model_mean
        model_for_mean <- model_mean
        model_for_sd <- model_mean
        model_for_ci <- model_mean
        ...
        for (i in as.numeric(veglevel)) {
           model_subset <- soc_den_for[,veg_case == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_for_mean[,cnt] <- model_subset
           } else {
              model_for_mean[,cnt] <- rowMeans(model_subset, na.rm=TRUE)
              model_for_sd[,cnt] <- apply(model_subset, 1, sd, na.rm=TRUE)
              model_for_ci[,cnt] <- 1.96 * model_for_sd[,cnt] / sqrt(rowSums(!is.na(model_subset)))
           }
           rm(model_subset)
           cnt <- cnt + 1
        }
        ...
        cnt <- 1
        for (i in as.numeric(ecolevel)) {
           model_subset <- soc_den_for[,eid_modeled == i]
           if(is.null(dim(model_subset))) {
             # model_subset[is.na(model_subset)] <- 0.
              model_for_mean[,cnt] <- model_subset
           } else {
...
## Write out model output into ascii file for Gelisol grouped by ecoregion or vegcover
if(group_data & group_model & write_text){
  depth_model <- model_ndepth*100
  text_name <- "proj_SOC_1cycle.csv"
  cname <- c("depth(cm)", "eid1_contemp", "tundra_contemp", "boreal_contemp", "eid1_forced", "tundra_forced", "boreal_forced", "eid1_free", "tundra_free", "boreal_free")
  write.csv(cbind(depth_model, model_mean, model_for_mean, model_free_mean), file=text_name, col.names=cname, row.names= FALSE, quote = FALSE)
}
...
if(group_data & group_model & group_figure){
   ...
   if(group_veg) {
    ...
    #  legend(108, 135, legend = c("   Soil Samples"), cex=3, pch = 17, bty = "n")
     ...
     # pdf(file="Grouped_ecoregion.pdf", width=17, height=10)
      pdf(file="Proj_10cycle.pdf", width=17, height=10)
      ### Failed to capture SOC density
      # Set margin
      par(omi=c(0,0.3,0,0), mar=c(2,8,8,2)) # all plots on one page
      # 
 ...
 #     lines(gelisol_mean[,3]-gelisol_sd[,3], depth_obs, lwd = 4, xaxt='n', yaxt='n', col="deepskyblue")
    ...
    #  lines(model_mean[,4], depth_model, lty = 6, lwd = 4, col="blue", xaxt='n', yaxt='n')    # Ecoregion 6 = Bering Tundra
      ...
      lines(model_for_mean[,2], depth_model, lty = 2, lwd = 4, pch=14, cex=1, col="red", xaxt='n', yaxt='n')    # Ecoregion 5= Arctic Tundra
      lines(model_for_mean[,3], depth_model, lty = 2, lwd = 4, pch=14, cex=1, col="green", xaxt='n', yaxt='n')    # Ecoregion 8 =Intermontane Boreal
   ...
   #   legend(60, 160, legend = c("Model - Arctic Tundra", "Model - Bering Tundra", "Model - Intermontane Boreal"), cex=3, lty=6, col=c("red","blue","green"), lwd=3, bty = "n")#, lty = 1:7,
      legend(90, 70, legend = c("Contemporary, Tundra", "Contemporary, Boreal"), cex=3, lty=3, col=c("red","green"), lwd=3, bty = "n")#, lty = 1:7,
      legend(90, 96, legend = c("1D forced, Tundra", "1D forced, Boreal"), cex=3, lty=2, col=c("red","green"), lwd=3, bty = "n")#, lty = 1:7,
      legend(90, 122, legend = c("1D coupled, Tundra", "1D coupled, Boreal"), cex=3, lty=1, col=c("red","green"), lwd=3, bty = "n")#, lty = 1:7,
           #  pch = "*", ncol = 4, cex = 0.8)
      # Put Label for X and Y axis
      mtext("Soil Depth (cm)", side=2, outer=TRUE, line=-2, cex=3)
      mtext("Organic Carbon Density (kgC/m3)", side=3, outer=TRUE,line=-3.7,cex=3)
