...
yield_newm3[N.isnan(yield_newm3)] = -9999
yield_newm3 = ma.masked_where(yield_newm3<=0,yield_newm3)
yield_newm3 = ma.masked_where(ncvar_maizearea<=0,yield_newm3)
yield_newm3=ma.filled(yield_newm3, fill_value=0.)

yield_new2m3[N.isnan(yield_new2m3)] = -9999
yield_new2m3 = ma.masked_where(yield_new2m3<=0,yield_new2m3)
yield_new2m3 = ma.masked_where(ncvar_maizearea<=0,yield_new2m3)
yield_new2m3=ma.filled(yield_new2m3, fill_value=0.)


yield_clmtfm3[N.isnan(yield_clmtfm3)] = -9999
yield_clmtfm3 = ma.masked_where(yield_clmtfm3<=0,yield_clmtfm3)
yield_clmtfm3 = ma.masked_where(ncvar_maizearea<=0,yield_clmtfm3)
yield_clmtfm3=ma.filled(yield_clmtfm3, fill_value=0.)
	    ...
	    for yy in range(0,4320):
	        if m3coun[xx,yy] >=couna and  m3coun[xx,yy] <=counb:
	            ...
	            m3harea=ncvar_maizearea[xx,yy]+ m3harea
                    yieldgi=yield_newm3[xx,yy]*ncvar_maizearea[xx,yy]+yieldgi
                    yieldgc=yield_clmtfm3[xx,yy]*ncvar_maizearea[xx,yy]+yieldgc
                    yieldgi1=yield_new2m3[xx,yy]*ncvar_maizearea[xx,yy]+yieldgi1
                    
	            a=a+1
        yieldagfi=yieldgi/m3harea
        yieldagfc=yieldgc/m3harea
        yieldagfi1=yieldgi1/m3harea
...
#name=["nigeria","thailand","japan","southkorea","france","southafrica","vietnam","russia","zimbabwe","iran","uganda"]
#range1=[33400,47500,43500,46700,42200,33900,48200,46100,34900,43000,34500]
#range2=[33400,47500,43500,46700,42200,33900,48200,46100,34900,43000,34500]
...
#means_fao=(8298*0.0001,14531*0.0001,19184*0.0001,13135*0.0001,25875*0.0001,16364*0.0001,12031*0.0001,10138*0.0001,22616*0.0001,15778*0.0001,12075*0.0001)
means_m3 = m3y
means_cesm=isamy1
...
#pro_fao=(429000,312432,235000,113196,201033,153472,149300,341920,143592,142000,128000)
pro_m3 = m3p
pro_cesm=isamp1
...
ha_ii = toarea
ha_fao = (74366591,29302790,13640026,8637503,9306760,6416600,1176460,1060700,8900,252647,616964)
#ha_fao=(517000,215014,122500,86176,77695,93787,124100,337260,63490,90000,106000)
ha_m3 = m3area
...
bar_width = 0.22
opacity = 0.8
rects0 = plt.bar(index, ha_fao, bar_width,
                 alpha=opacity,
                 color='k',
                 label='FAO')
rects2 = plt.bar(index+bar_width, ha_m3, bar_width,
                 alpha=opacity,
                 color='c',
                 label='M3')
rects1 = plt.bar(index+bar_width*2, ha_ii, bar_width,
                 alpha=opacity,
                 color='r',
                 label='Iizumi')
...
plt.xlabel('Country',fontsize=18)
plt.ylabel('Harvested area (ha)',fontsize=18)
plt.title('2000',fontsize=18)
plt.xticks(index + bar_width+0.1, ('Global','USA','Brazil','Argentina','China','India','Paraguay','Canada','Uruguay','Italy','Bolivia'))
#plt.xticks(index + bar_width+0.1, ("Nigeria","Thailand","Japan","South Korea","France","South Africa","Vietnam","Russia","Zimbabwe","Iran","Uganda"))
