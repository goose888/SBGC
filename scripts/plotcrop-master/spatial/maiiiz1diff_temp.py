...
for i, year in enumerate(years):
    ...
    yield2 = basei.variables["g_Temp"][7,0,:,:]
    yieldf2[i, :, :] = yield2
    ...
    yield12 = base1.variables["g_Temp"][7,0,:,:]
    yieldfi[i, :, :] = yield12
    ...
    yield23 = basei1.variables["g_Temp"][7,0,:,:]
    yieldf2i[i, :, :] = yield23
...
ax.set_title("ISAM CESM-NCEP Maize T (K)",fontsize=18)
...
map.drawcountries()
map.drawmapboundary()
cs = map.pcolormesh(x,y,cc,cmap=plt.cm.bwr,vmin=-8,vmax=8)
cbar = map.colorbar(cs,location='bottom',size="5%",pad="2%")
cbar.ax.tick_params(labelsize=15)
plt.axis('off')
#plt.savefig('maiiizisam.jpg')
plt.savefig('maiiizisam_temp100.jpg',dpi=300,bbox_inches='tight')
