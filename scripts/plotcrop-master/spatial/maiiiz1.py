...
ax.set_title("Iizumi Maize Yield gridecll (g/$\mathregular{m^2}$)",fontsize=20)
...
map.drawcountries()
map.drawmapboundary()
cs = map.pcolormesh(x,y,iyield*iarea/100/10*1000,cmap=plt.cm.YlGn,norm=colors.PowerNorm(gamma=1./2.),vmin=0,vmax=100)

cbar = map.colorbar(cs,location='bottom',size="5%",pad="2%")
cbar.ax.tick_params(labelsize=15)
plt.axis('off')
plt.savefig('maiiiz.jpg',dpi=300,bbox_inches='tight')

fig = plt.figure(figsize=(20,15))


ax2 = fig.add_subplot(321)
ax2.set_title("ISAM-CESM Maize Yield gridcell (g/$\mathregular{m^2}$)",fontsize=20)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
x,y = map(lon2,lat2)
iyield = ma.masked_where(iyield<=0,iyield)
iarea = ma.masked_where(iarea<=0,iarea)
...
cs1 = map.pcolormesh(x,y,yield_new*iarea/100/10*1000,cmap=plt.cm.YlGn,norm=colors.PowerNorm(gamma=1./2.),vmin=0,vmax=100)
...
cs1 = map.pcolormesh(x,y,yield_clmtf*iarea/100/10*1000,cmap=plt.cm.YlGn,norm=colors.PowerNorm(gamma=1./2.),vmin=0,vmax=100)
...
ax2.set_title("ISAM-Iizumi Maize Yield gridcell (g/$\mathregular{m^2}$)",fontsize=20)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
#map = Basemap(projection='robin',lon_0=0,resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
cs1 = map.pcolormesh(x,y,(yield_new*iarea/100/10*1000)-(iyield*iarea/100/10*1000),cmap=plt.cm.bwr,vmin=-100,vmax=100)
...
ax2.set_title("CLM-Iizumi Maize Yield gridcell (g/$\mathregular{m^2}$)",fontsize=20)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
#map = Basemap(projection='robin',lon_0=0,resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
cs1 = map.pcolormesh(x,y,(yield_clmtf*iarea/100/10*1000)-(iyield*iarea/100/10*1000),cmap=plt.cm.bwr,vmin=-100,vmax=100)
