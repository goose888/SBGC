...
for i, year in enumerate(years):
    ...
    yield1 = base.variables["g_Precip"][7,1,:,:]
    yieldf[i, :, :] = yield1
    ...
    yield2 = basei.variables["g_Precip"][7,1,:,:]
    yieldf2[i, :, :] = yield2
    ...
    yield12 = base1.variables["g_Precip"][7,1,:,:]
    yieldfi[i, :, :] = yield12
    ...
    yield23 = basei1.variables["g_Precip"][7,1,:,:]
    yieldf2i[i, :, :] = yield23
...
ax.set_title("ISAM CESM-NCEP P (mm/yr)",fontsize=18)
...
map.drawcountries()
map.drawmapboundary()
cs = map.pcolormesh(x,y,cc,cmap=plt.cm.bwr,vmin=-400,vmax=400)
cbar = map.colorbar(cs,location='bottom',size="5%",pad="2%")
cbar.ax.tick_params(labelsize=15)
plt.axis('off')
#plt.savefig('maiiizisam.jpg')
plt.savefig('soyiizisam_p100.jpg',dpi=300,bbox_inches='tight')
