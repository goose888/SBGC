...
#    base = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/model/global/isam_future/rcp45bm/output/rcp45bm.bgp-yearly_crop_{0}.nc".format(year), mode='r')
    ...
    yieldf[i, :, :] = yield1
    yield11 = basea.variables["g_Temp"][7,0,:,:]
    yieldf1[i, :, :] = yield11
...
cs = map.pcolormesh(x,y,cisa,cmap=plt.cm.bwr,vmin=-0.5,vmax=0.5)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')
...
cas= ma.masked_where(cas==0.,cas)
cs = map.pcolormesh(x,y,cas,cmap=plt.cm.bwr,vmin=-0.5,vmax=0.5)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')

plt.savefig('scp45maizetddiff1.jpg',dpi=300,bbox_inches='tight')
plt.show()
