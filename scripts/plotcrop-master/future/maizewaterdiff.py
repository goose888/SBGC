...
#    base = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/model/global/isam_future/rcp45dm/output/rcp45dm.bgp-yearly_crop_{0}.nc".format(year), mode='r')
    ...
    yieldf[i, :, :] = yield1
    yield11 = basea.variables["g_rootwater"][7,0,:,:]
    yieldf1[i, :, :] = yield11
...
#    base2 = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/model/global/isam_future/rcp45dm/output/rcp45dm.bgp-yearly_crop_{0}.nc".format(year1), mode='r')    
    ...
    yield21 = base2a.variables["g_rootwater"][7,0,:,:]
    yieldf21[i, :, :] = yield21
...
ax2.set_title("ISAM root water difference 2010s (t/ha)",fontsize=18)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
...
cs = map.pcolormesh(x,y,cisa,cmap=plt.cm.bwr,vmin=-10,vmax=10)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')
...
ax5.set_title("ISAM g_rootwater difference 2050s (t/ha)",fontsize=18)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
...
cas= ma.masked_where(cas==0.,cas)
cs = map.pcolormesh(x,y,cas,cmap=plt.cm.bwr,vmin=-10,vmax=10)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')

plt.savefig('scp45maizewatercdiff1.jpg',dpi=300,bbox_inches='tight')
plt.show()
