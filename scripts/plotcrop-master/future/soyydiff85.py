...
cs = map.pcolormesh(x,y,cisa,cmap=plt.cm.bwr,vmin=-1,vmax=1)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')
...
ax5.set_title("ISAM yield difference 2050s (t/ha)",fontsize=18)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
...
plt.savefig('scp85soyybdiff.jpg',dpi=300,bbox_inches='tight')
plt.show()
