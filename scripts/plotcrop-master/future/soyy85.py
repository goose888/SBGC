...
#    base = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/model/global/isam_future/rcp85bs/output/rcp85bs.bgp-yearly_crop_{0}.nc".format(year), mode='r')
...
#    base2 = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/model/global/isam_future/rcp85bs/output/rcp85bs.bgp-yearly_crop_{0}.nc".format(year1), mode='r')
...
map.drawmapboundary()
clmhis= ma.masked_where(clmhis<=0.,clmhis)
cs = map.pcolormesh(x,y,clmhis,cmap=plt.cm.jet,vmin=0,vmax=6)
...
cs = map.pcolormesh(x,y,yield_new,cmap=plt.cm.jet,vmin=0,vmax=6)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')
...
map.drawmapboundary()
clmfuture= ma.masked_where(clmfuture<=0.,clmfuture)
cs = map.pcolormesh(x,y,clmfuture,cmap=plt.cm.jet,vmin=0,vmax=6)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')
...
ax5.set_title("ISAM yield 2050s (t/ha)",fontsize=18)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
yield_new2= ma.masked_where(yield_new2<=0.,yield_new2)
cs = map.pcolormesh(x,y,yield_new2,cmap=plt.cm.jet,vmin=0,vmax=6)
cbar = map.colorbar(cs,location='bottom',size="4%",pad="2%")
cbar.ax.tick_params(labelsize=12)
plt.axis('off')

plt.savefig('scp85soyyd1.jpg',dpi=300,bbox_inches='tight')
plt.show()
