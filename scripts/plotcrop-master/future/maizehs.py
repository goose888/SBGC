...
#    base2 = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/model/global/isam_future/rcp45cm/output/rcp45cm.bgp-yearly_crop_{0}.nc".format(year1), mode='r')
    ...
    yieldf2[i, :, :] = yield2
    yield21 = base2.variables["fhsa"][0,:,:]
    hs2[i, :, :] = yield21
...
ax2.set_title("ISAM heat stress 2010s ",fontsize=18)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
...
ax5.set_title("ISAM heat stress 2050s ",fontsize=18)
map = Basemap(projection ='cyl', llcrnrlat=-65, urcrnrlat=90,llcrnrlon=-180, urcrnrlon=180, resolution='c')
map.drawcoastlines()
map.drawcountries()
map.drawmapboundary()
...
plt.savefig('scp85maidhs.jpg',dpi=300,bbox_inches='tight')
plt.show()
